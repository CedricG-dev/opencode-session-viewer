...
| Data & formats (ids, dates, error shapes, envelopes) | `status` ∈ `"idle" \| "busy" \| "retry"` (never `"error"` — errors are `errorFlag`/`errorMessage`, per PRD glossary). `cost` = `{ tokens: number, dollars: number }`. `lastActivity` = ISO 8601 UTC string. SSE envelope: first `data:` line on any connection is the full view-model array (AD-7 snapshot); every subsequent `data:` line is a single full-replacement session view model (AD-7 delta). |

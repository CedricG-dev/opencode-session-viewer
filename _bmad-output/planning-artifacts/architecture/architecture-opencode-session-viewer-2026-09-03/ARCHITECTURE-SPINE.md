...
| Concern | Convention |
| --- | --- |
| Naming (entities, files, interfaces, events) | Per-session view-model fields: `id, title, status, tokens, cost, models, ownTokens, ownCost, ownModels, messageCount, lastActivity, errorFlag, errorMessage?` (camelCase). `id` is opencode's own session ID string, verbatim — never regenerated or aliased. `models`/`ownModels` are `ModelUsage[]` (`{ providerID, modelID, tokens, cost }`), grouped by the raw `providerID`/`modelID` opencode reports on each assistant message — no display-name catalog lookup. |
...
- **Per-model cost breakdowns** (PRD §5.2) — done: `ModelUsage[]` on `ViewModel`/`SubSessionSummary` (`core/view-model.ts`), surfaced via `dashboard/components/ModelUsageList.ts` at the top-level aggregate card only (never filtered). Session/sub-session cards instead let the user drill into a specific model via `dashboard/components/ModelFilterBar.ts`, which re-sums each session's already-reported per-model data client-side (`dashboard/model-filter.ts`) rather than duplicating a static per-card breakdown.
- **Cost trend charts, budget alerts** (PRD §5.2) — when built, they extend `core/view-model.ts` and `dashboard/components/`; no paradigm change anticipated, but not designed here.
